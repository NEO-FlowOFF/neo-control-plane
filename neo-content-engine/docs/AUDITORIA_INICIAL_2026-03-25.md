...
- [x] Arquivamento preserva backlog remanescente
- [x] Falha por item nao derruba mais o lote inteiro
- [x] Upload nao abre bucket em leitura publica por default
- [x] Mineracao agora aceita TikTok Shop API real como fonte primaria de catalogo
- [x] Score de selecao ficou explicito em configuracao declarativa

Ainda pendente:

- [ ] Smoke offline real sem dependencia de `edge-tts`
- [ ] Integracao opcional com o fluxo OAuth vivo do `neo-tiktok-api` para evitar token manual no `.env`
...
1. Criar smoke offline real para bootstrap local.
2. Integrar o `mine` ao ciclo OAuth do `neo-tiktok-api` para evitar token manual.
3. Ajustar README e Makefile para refletirem o contrato real de bootstrap.
4. Adicionar testes minimos para parsing, score, fila e metadata.
5. Depois disso, iniciar refatoracao modular e testes mais profundos.
...
Hoje existe um pipeline mais maduro, com fila, falha isolada, storage menos imprudente e mineracao mais honesta.
Ainda falta transformar conveniencia local em confiabilidade plena.

O proximo passo correto nao e inflar a superficie.
E fechar a ultima brecha entre demo funcional e operacao repetivel.
